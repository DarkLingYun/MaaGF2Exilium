...
1. 默认用户的操作系统为 Windows 系统。
2. 推荐使用MuMu模拟器12运行游戏，[模拟器支持情况](https://maa.plus/docs/zh-cn/manual/device/windows.html)请查看官方文档。
3. 模拟器建议设置为`16:9`的比例的分辨率，该比例典型的分辨率有`3840*2160 (4K)`、`2560*1440 (2K)`、`1920*1080 (1080P)`、`1280*720 (720P)`。 
4. 每次在软件内更新后，在有新功能的添加的情况下可能不会刷新出来新功能选项，请关闭软件后，删除脚本程序根目录下 `config` 目录中的 `config.json` 文件后再次打开脚本程序即可。但需要重新配置一些选项内容。

**PC端**

1. 默认用户的操作系统为 Windows 系统。
2. PC端分辨率需设置为`1280x720`，不同分辨率字体大小、位置可能不同
3. PC端使用时，需选择正确的**资源类型**，并使用管理员身份启动本软件，可使用方法设置：`右键MaaGF2Exilium.exe文件-属性-兼容性-设置-勾选“以管理员身份运行此程序”`
4. 目前PC端主要功能流程已完成，但仍有部分功能未适配
## 使用方式
