...
# 如果设置了 PAT，将使用它来进行 API 请求和下载，可以提高速率限制
# 留空字符串则不使用认证
GITHUB_PAT = ""
    ...
    try:
        if filename.endswith(".zip"):
            with zipfile.ZipFile(file_path, "r") as zip_ref:
                ...
                with tqdm(
                    total=len(file_list), desc="Extracting", unit="files"
                ) as pbar:
...
def download_maa_framework(platform_name, arch_name, output_path="deps"):
    ...
    try:
        ...
        # 下载文件
        response = requests.get(download_url, stream=True, headers=get_github_headers())
        response.raise_for_status()
